...
def extract_from_googledrive(id, filename, dirname):
    errmsg = download_googledrive(id, filename)
...
#        self.button_update = wx.Button(panel, wx.ID_ANY, 'Update')
#        self.button_uninstall = wx.Button(panel, wx.ID_ANY, 'Uninstall')
...
#        hsizer.Add(self.button_update, 0, wx.ALL, 10)
#        hsizer.Add(self.button_uninstall, 0, wx.ALL, 10)
...
#        self.button_uninstall.Enable()
    ...
    def click_install(self, event):
        ...
        thread = Thread(target=self.install_all)
        thread.start()
    ...
    def install_mannequinchallenge(self):
        ...
        url = 'https://github.com/errno-mmd/mannequinchallenge-vmd/archive/{}.zip'.format(tag)
    ...
    def run_command(self, command, **kwargs):
        ...
        self.subproc = subprocess.Popen(command.split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True, bufsize=1, encoding='utf-8', **kwargs)
